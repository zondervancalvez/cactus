...
import { PluginConsortiumIndy } from "../plugin-consortium-indy";

export interface IGetNodeJwsEndpointOptions {
  plugin: PluginConsortiumIndy;
  keyPairPem: string;
...
export class GetNodeJwsEndpoint implements IWebServiceEndpoint {
  ...
  private readonly plugin: PluginConsortiumIndy;
  ...
  constructor(public readonly options: IGetNodeJwsEndpointOptions) {
    ...
    Checks.truthy(
      options.plugin instanceof PluginConsortiumIndy,
      `${fnTag} options.plugin instanceof PluginConsortiumIndy`,
    );
