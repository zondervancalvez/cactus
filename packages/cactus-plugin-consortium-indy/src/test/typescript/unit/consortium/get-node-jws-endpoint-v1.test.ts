...
test("Can provide JWS", async (t: Test) => {
  ...
  // Creating the PluginConsortiumIndy object to observe the prometheus metrics.
  const options: IPluginConsortiumIndyOptions = {
    instanceId: uuidv4(),
    keyPairPem: keyPairPem,
    consortiumDatabase: db,
  };

  const PluginConsortiumIndy: PluginConsortiumIndy = new PluginConsortiumIndy(
    options,
  );
  ...
  t.comment(
    `Metrics URL: ${apiHost}/api/v1/plugins/@hyperledger/cactus-plugin-consortium-indy/get-prometheus-exporter-metrics`,
  );

  const config = new Configuration({ basePath: apiHost });
  const apiClient = new ConsortiumManualApi(config);

  await PluginConsortiumIndy.getOrCreateWebServices();
  await PluginConsortiumIndy.registerWebServices(expressApp);

  const pubKeyPem = keyPair.toPEM(false);

  const jws = await PluginConsortiumIndy.getNodeJws();
